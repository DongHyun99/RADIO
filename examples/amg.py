...
def write_masks_to_folder(masks: List[Dict[str, Any]], image, path: str) -> None:
    header = "id,area,bbox_x0,bbox_y0,bbox_w,bbox_h,point_input_x,point_input_y,predicted_iou,stability_score,crop_box_x0,crop_box_y0,crop_box_w,crop_box_h"  # noqa
    metadata = [header]
    for i, mask_data in enumerate(masks):
        mask = mask_data["segmentation"]
        filename = f"{i}.png"
        cv2.imwrite(os.path.join(path, filename), np.expand_dims(mask, axis=2) * image)
        mask_metadata = [
            str(i),
            str(mask_data["area"]),
...
def main(args: argparse.Namespace) -> None:
    ...
    if args.model_type != 'radio':
        ...
        sam = sam_model_registry["vit_h"](checkpoint="/lustre/fsw/portfolios/llmservice/projects/llmservice_nlp_fm/model_zoo/sam/sam_vit_h_4b8939.pth")
    ...
    for t in targets:
        ...
        if output_mode == "binary_mask":
            ...
            write_masks_to_folder(masks, image, save_base)
        else:
